...
export async function sendProjectToTelegram(project: ProjectData): Promise<boolean> {
  try {
    ...
    if (project.description) {
      message += `📄 <b>${project.description.substring(0, 200)}${project.description.length > 200 ? '...' : ''}</b>\n\n`
    }
    ...
    if (project.githubUrl) {
      message += `\n🔗 <a href="${project.githubUrl}">查看源码</a>`
    }
...
export async function sendPostToTelegram(post: PostData): Promise<boolean> {
  try {
    ...
    message += `📄 <b>${post.content.substring(0, 300)}${post.content.length > 300 ? '...' : ''}</b>\n\n`
 ...
 * 获取 Telegram 配置状态
 */
export async function getTelegramStatus(): Promise<{ enabled: boolean; configured: boolean; channel?: string }> {
  const config = await getEnabledConfig()
